...
window.onload = function init() {
    ...
    plane = new Plane(cam, 1);
    cube = new Cube(cam, "crate_texture")
...
class Plane {
    constructor(cam, subdivs) {
        ...
        this.program = initShaders(gl, "vShaderT.glsl", "fShaderT.glsl");
        gl.useProgram(this.program);

        var image = new Image();
        var self = this; //for reference inside onload function
        image.onload = function(){
            self.textureID = gl.createTexture();
            gl.bindTexture(gl.TEXTURE_2D, self.textureID);
            gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGB, this.width, this.height, 0, gl.RGB, gl.UNSIGNED_BYTE, image);
    ...
    divideQuad(a, b, c, d, depth) {
        if (depth > 0) {
            ...
            //Triangle #1
            this.triangle(a, b, c);
            this.vTexs.push(vec2(0.0, 0.0)); //first vertex
            this.vTexs.push(vec2(1.0, 0.0)); //second vertex
            this.vTexs.push(vec2(1.0, 1.0)); //third vertex
            //Triangle #2
            this.triangle(c, d, a);
            this.vTexs.push(vec2(0.0, 0.0)); //first vertex
            this.vTexs.push(vec2(1.0, 1.0)); //second vertex
            this.vTexs.push(vec2(0.0, 1.0)); //vertex
        }
    }
...
class SkyBox {
    constructor(cam) {
        ...
        // Load shaders and initialize attribute buffers
        this.program = initShaders(gl, "vShaderS.glsl", "fShaderS.glsl");
        gl.useProgram(this.program);

        var faces = [
            {
                target: gl.TEXTURE_CUBE_MAP_POSITIVE_X,
                src: "./textures/sky-right.jpg" 
            },
            {
                target: gl.TEXTURE_CUBE_MAP_NEGATIVE_X,
                src: "./textures/sky-left.jpg" 
            },
            {
                target: gl.TEXTURE_CUBE_MAP_POSITIVE_Y,
                src: "./textures/sky-top.jpg" 
            },
            {
                target: gl.TEXTURE_CUBE_MAP_NEGATIVE_Y,
                src: "./textures/sky-bottom.jpg" 
            },
            {
                target: gl.TEXTURE_CUBE_MAP_POSITIVE_Z,
                src: "./textures/sky-front.jpg" 
            },
            {
                target: gl.TEXTURE_CUBE_MAP_NEGATIVE_Z,
                src: "./textures/sky-back.jpg" 
            }
        ];
        faces.forEach(face => {
            var image = new Image();
            ...
            image.onload = function(){
                ...
                gl.texImage2D(face.target, 0, gl.RGB, this.width, this.height, 0, gl.RGB, gl.UNSIGNED_BYTE, image);
        ...
        this.camMatrix =  lookAt(vec3(0,0,0), subtract(vec3(0,0,0), cam.n), cam.v);
        this.camMatrixID = gl.getUniformLocation(this.program, "camMatrix");
    }
...
class Cube {
    ...
    draw() {
        ...
        gl.enableVertexAttribArray(this.aTCoord);
        gl.drawArrays(gl.TRIANGLES, 0, 36);
        gl.disableVertexAttribArray(this.aTCoord);
        gl.disableVertexAttribArray(this.aPosition);
        gl.disableVertexAttribArray(this.aNormal); 
    }
}
...
document.addEventListener('keyup', event => {
    ...
    cam.updateCamMatrix();
    skybox = new SkyBox(cam.camMatrix);
